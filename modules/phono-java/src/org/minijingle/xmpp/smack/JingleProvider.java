...
public class JingleProvider implements IQProvider, PacketInterceptor {
    ...
    public JingleProvider(boolean ice) {
        super();
        _ice = ice;
    }
    ...
    public void enableJingle(final Connection connection) {
        ...
        if (_ice) {
            JingleIQ.getStream().alias("transport", IceUdpTransport.class);
        } else {
    ...
    public void interceptPacket(Packet packet) {
        if ((packet instanceof Presence)) {
            packet.addExtension(new PacketExtension() {
                ...
                public String toXML() {
                    return "<c xmlns=\"http://jabber.org/protocol/caps\" node=\"phono-jingle\" ver=\"0.1\" ext=\"\"></c>";
                }
            });
        }
    }
